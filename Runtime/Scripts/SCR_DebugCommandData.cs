    ...
    public static class DebugCommandData
    {
        ...
        private static bool ExecuteInternal(string commandInput, List<DebugCommandInstanceBase> commandData)
        {
            ...
            for (int i = 0; i < commandData.Count; i++)
            {
                ...
                else if (commandInstance as DebugCommandInstance<int, float> != null)
                {
                    if (properties.Length != 3)
                    {
                        break;
                    ...
                    (commandInstance as DebugCommandInstance<int, float>).Execute(int.Parse(properties[1]), float.Parse(properties[2]));
                    return true;
                }
        ...
        {
            GameManager.Instance.WaitUntil(WaitResume, () => GameManager.Instance.SetTimeScale(scale));
