...
namespace Core.Misc
{
    ...
    public class InteractionController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            wishHold = ManagerCoreInput.Instance.KeyInteract.IsPressing;
            wishInteract = ManagerCoreInput.Instance.KeyInteract.IsPressedDown;
            canInteract = wishInteract && interactTimer > 0.01f;
            canHold = wishHold && !wasInteractHold;

            if (wasInteractHold && ManagerCoreInput.Instance.KeyInteract.IsPressedUp)
            {
                wasInteractHold = false;
            }
            if (isHolding)
            {
                if (ManagerCoreInput.Instance.KeyInteract.IsPressedUp)
                {
                    isHolding = false;
